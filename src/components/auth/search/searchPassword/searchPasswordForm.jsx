...
const SearchPasswordForm = () => {
  ...
  const validatePassword = (password) => {
    ...
    if (!hasUpperCase) return '대문자를 1자 이상 포함해야 합니다.';
    if (!hasMinLength) return '비밀번호는 최소 8자리 이상이어야 합니다.';
    return null;
  };
  ...
  const passwordRequiredError =
    isVerified && password.trim().length === 0
      ? '비밀번호를 입력해주세요.'
      : isVerified && password.trim().length > 0
      ? validatePassword(password)
      : null;
  const passwordCheckRequiredError =
    isVerified && passwordCheck.trim().length === 0
      ? '비밀번호를 다시 입력해주세요.'
      : null;
  ...
  const getErrorStyle = (field) =>
    errors[field] ? { borderColor: colors.searchRed } : {};
    ...
    <s.SearchFormContainer>
      ...
      <s.SearchInnerContainer>
        ...
        <s.SearchItemContainer>
          ...
          <s.SearchIdButton onClick={handleSubmit(onSubmit)}>
            인증번호 발송
          </s.SearchIdButton>
        </s.SearchItemContainer>
        ...
        <s.SearchItemContainer>
          ...
          <SearchInput
            placeholder="이메일로 발송한 6자리 숫자를 입력해주세요."
            value={enteredCode}
      ...
      <s.SearchInnerContainer2>
        <s.SearchItemContainer>
          ...
          <SearchInput
            ...
            disabled={!isVerified}
            {...register('password')}
            style={passwordRequiredError ? { borderColor: 'red' } : {}}
          />
          {passwordRequiredError && (
            <s.ErrorP>{passwordRequiredError}</s.ErrorP>
          )}
        </s.SearchItemContainer>
        ...
        <s.SearchItemContainer>
          ...
          <SearchInput
            ...
            style={
              passwordCheckRequiredError || passwordMatchError
                ? { borderColor: 'red' }
                : {}
            }
          />
          {passwordCheckRequiredError && (
            <s.ErrorP>{passwordCheckRequiredError}</s.ErrorP>
          )}
      ...
      <SearchButton
        ...
        onClick={handleSearchPasswordClick}
      >
        완료
      </SearchButton>
    </s.SearchFormContainer>
