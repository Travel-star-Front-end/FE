...
const CalenderLeft = ({ selectedDay, setSelectedDay }) => {
  ...
  return (
    <CalenderLeftContainer>
      <CalendarContainer>
        <Header>
          <CalenderP>{year}년 {month + 1}월</CalenderP>
          <ButtonContainer>
            <ButtonImg src={CalenderButton} alt="button" onClick={() => changeMonth(-1)} />
            <ButtonImg src={CalenderButton} alt="button" style={{ rotate: "180deg" }} onClick={() => changeMonth(1)} />
          </ButtonContainer>
        </Header>
        ...
        <DateContainer>
          {calendarDates.map((date, index) => {
            ...
            return (
              <DateCell key={index} className={currentMonth ? "" : "inactive"} onClick={() => currentMonth && handleDateClick(day, cellMonth, cellYear)} lastrow={lastrow.toString()} selected={selected}>
